...
class SendDealIsNotFinishedFirstNotificationAction extends Action
{
    public function run()
    {
        $deals = app(GetAllDealsTask::class)
            ->createdAt(config('notifications.deal.notFinished.first_reminder'))
            ->notStarted()
            ->liveDeal()
            ->run();
        ...
        foreach ($deals as $deal) {
            ...
            try {
                app(NotificationTask::class)->run($deal, MailContext::DEAL_IS_NOT_COMPLETED_FIRST_REMINDER, $data);
            } catch (\Exception $e) {
                \Log::error(sprintf('[deal_id: %s] error sending first deal notification: %s', $deal->id, $e->getMessage()));
            }
        }
    }
}
