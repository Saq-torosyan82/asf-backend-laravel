...
class GetBorrowerDocumentsTask extends Task
{
    ...
    {
        // get user type
        $user_type = app(GetBorrowerTypeTask::class)->run($userId);

        // get available contract types
        $contractTypes = ContractType::getBorrowerAvailableContracts($user_type);

        $all_documents = [];
        foreach ($contractTypes as $type => $unit) {
            $deal_documents = UserDocumetType::getRequiredDealDocuments($type);
            foreach ($deal_documents as $doc_type) {
                if (isset($all_documents[$doc_type])) {
                    continue;
                }

                $all_documents[$doc_type] = $type;
            }
        }
        ...
        $data = [];
        foreach ($all_documents as $type => $contractType) {
            $id = '';
            $url = '';
            $file_type = '';
            $file_name = '';
