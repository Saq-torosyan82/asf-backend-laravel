...
class FormatReadableLenderDealCriteriaTask extends Task
{
    use HashIdTrait;
    ...
    public function run($user_id)
    {
        ...
        foreach ($dealCriteria as $deal) {
            ...
            foreach ($keys as $k => $v) {
                $row[$k] = $criteriaData[$deal->$v]->value;
            }
            $row['note'] = $deal->note;
            ...
            {
               $row['currency'][] = $criteriaData[$rrow->currency_id]->value;
            }

            // get countries
            foreach($deal->countries as $rrow)
            {
                $row['country'][] = $countryData[$rrow->country_id]->name;
            }

            // get sports
            foreach($deal->sports as $rrow)
            {
                $row['sport'][] = $sportData[$rrow->sport_id]->name;
            }
