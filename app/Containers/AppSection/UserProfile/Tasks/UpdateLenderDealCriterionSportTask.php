...
class UpdateLenderDealCriterionSportTask extends Task
{
    ...
    public function run($criterion_id, $values)
    {
        try {
            ...
            foreach ($values as $row) {
                ...
                // save new value
                app(CreateLenderDealCriterionSportTask::class)->run($criterion_id, $row['id']);
            }

            // delete ramained countries
            foreach ($cData as $key => $unit) {
                $this->repository->deleteWhere(['criterion_id' => $criterion_id, 'sport_id' => $key]);
            }
